...
import { getAuthorBySlug, getAllAuthors } from '../../../services/authorService';
...
export default async function AuthorDetailPage({ params }: Props) {
  const { slug } = await params;
  const [author, allArticles] = await Promise.all([getAuthorBySlug(slug), getArticles()]);
    ...
    '@graph': [
      {
        ...
        itemListElement: [
          ...
          { '@type': 'ListItem', position: 2, name: 'Expertos', item: `${BASE_URL}/authors` },
          { '@type': 'ListItem', position: 3, name: author.name, item: `${BASE_URL}/author/${author.slug}` },
        ],
      },
      {
        '@type': 'ProfilePage',
        mainEntity: {
          ...
          sameAs: [author.socials?.linkedin, author.socials?.twitter, author.socials?.instagram, 'https://www.youtube.com/@thegarfieldofseo'].filter(Boolean),
        },
      },
    ],
  };
          ...
          <div className="space-y-6">
            {author.externalPublications.map((pub, i) => (
              <a key={i} href={pub.link} target="_blank" rel="noopener noreferrer" className="group flex flex-col sm:flex-row sm:items-center justify-between p-6 rounded-2xl bg-white border border-gray-100 hover:border-garfield-200 hover:shadow-lg transition-all">
                <div>
                  <h3 className="text-xl font-bold text-slate-900 group-hover:text-garfield-600 mb-1">{pub.title}</h3>
                  <div className="flex items-center gap-3 text-sm text-slate-500">
                    <span className="font-semibold text-slate-700">{pub.medium}</span><span>•</span><span>{pub.date}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2 text-sm font-bold text-slate-400 group-hover:text-garfield-500 uppercase tracking-widest mt-4 sm:mt-0">

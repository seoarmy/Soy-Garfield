      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex items-center gap-4 py-3">
          ...
          <div className="flex-1 min-w-0">
            {isExternal ? (
              <a href={newsLink} target="_blank" rel="noopener noreferrer" className="group flex items-center gap-3 hover:opacity-80 transition-opacity">
                <span className="text-sm font-bold text-white truncate">{currentNews.title}</span>
                <ExternalLink size={14} className="text-slate-400 flex-shrink-0" />
              </a>
            ) : (
              <Link href={newsLink} className="group flex items-center gap-3 hover:opacity-80 transition-opacity">
                <span className="text-sm font-bold text-white truncate">{currentNews.title}</span>
